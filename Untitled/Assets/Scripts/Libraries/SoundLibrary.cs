...
public class SoundLibrary : MonoBehaviour
{
    ...
    void Awake() {
        // 2D
        _sfxList2D = Resources.LoadAll("SFX2D/local" , typeof(AudioClip)).Cast<AudioClip>().ToArray(); // load all of sfx in resources folder ( thats why this folder is called resources )
        for (int i = 0; i < _sfxList2D.Length; i++){
            sfxLibrary2D.Add(_sfxList2D[i].name ,_sfxList2D[i]);
        }
        // 3D
        _sfxList3D = Resources.LoadAll("SFX3D/local" , typeof(AudioClip)).Cast<AudioClip>().ToArray(); // load all of sfx in resources folder ( thats why this folder is called resources )
        for (int i = 0; i < _sfxList3D.Length; i++){
            sfxLibrary3D.Add(_sfxList3D[i].name ,_sfxList3D[i]);
        }

        AudioClip[] _sfxInFolder;
        // 2D Random
        foreach (string folder in registered2D){
            _sfxInFolder = Resources.LoadAll("SFX2D/" + folder, typeof(AudioClip)).Cast<AudioClip>().ToArray();
            sfxList2DRandom.Add(folder ,_sfxInFolder);
        }
        // 3D Random
        foreach (string folder in registered3D){
            _sfxInFolder = Resources.LoadAll("SFX3D/" + folder, typeof(AudioClip)).Cast<AudioClip>().ToArray();
